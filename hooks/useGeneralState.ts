...
export const useGeneralState = () => {
  const {value: storageValue, updateValue} = useLocalStorage<{columnConfig?: number, rowConfig?: number, highlightCount?: number} | null>(STORAGE_CONFIG_DATA, null);
  ...
  const handleSetHighlightCount = (value: number) => {
